...
# -----------------------------
# Configuration (AFTER all function definitions)
# -----------------------------
...
client = OpenAI(
    api_key=os.environ.get("DEEPSEEK_API_KEY"),  # Set DEEPSEEK_API_KEY in environment
...
# Find clang-tidy (NOW THIS WORKS because function is defined above)
...
for task_index, (category, task_id, query) in enumerate(tasks, start=1):
    ...
    with open(metadata_csv, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for run in range(1, RUNS_PER_TASK + 1):
            ...
            # DeepSeek API call (same interface as OpenAI)
