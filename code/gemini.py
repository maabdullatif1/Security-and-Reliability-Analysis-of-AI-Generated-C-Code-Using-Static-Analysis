...
# -------------------------------------------------
# macOS SDK path helper (fixes 'iostream' not found for clang-tidy on macOS)
# -------------------------------------------------
...
# -----------------------------
# Configuration (Gemini via OpenRouter)
# -----------------------------
MODEL = "google/gemini-2.0-flash-001"   # OpenRouter model string you already used
RUNS_PER_TASK = 10
TASKS_FILE = "tasks.csv"  # required columns: category, task_id, task_description
BASE_DIR = "experiments_gemini_openrouter"
EXEC_DATE = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

EXCEL_OUT = os.path.join(BASE_DIR, "static_analysis_summary.xlsx")

# clang-tidy checks (same as your pipeline)
CLANG_TIDY_CHECKS = "clang-analyzer-*,bugprone-*,performance-*"

# OpenRouter headers (required/standard)
OPENROUTER_HEADERS = {
    "HTTP-Referer": "http://localhost:3000",
    "X-Title": "Security & Reliability Study Script",
}

# Rate-limit safety (free/low-tier often needs pauses)
SLEEP_BETWEEN_REQUESTS = 3  # increase if you still hit 429


# -----------------------------
# Client (OpenRouter key from env, no key in code)
# -----------------------------
...
with open(TASKS_FILE, "r", encoding="utf-8-sig", newline="") as f:
    ...
    for row in reader:
        ...
        query = (row.get("task_description") or "").strip()
...
if not CLANG_TIDY_PATH:
    print("[WARN] clang-tidy not found. Clang-Tidy will be skipped.", flush=True)
else:
...
if sdk_check:
    ...
    print("[WARN] Could not determine macOS SDK path via xcrun. If clang-tidy can't find <iostream>, run: xcode-select --install", flush=True)
